...
/**
 * Типы данных под НОВЫЙ ответ бэкенда
 * {
 *   "Философия": { reply: string, book_search: Book[], vector_search: Book[] },
 *   "История Казахстана": { ... },
 *   ...
 * }
 */
  ...
  useEffect(() => {
    async function load() {
      try {
        setLoading(true);
        setError(null);
        // ВАЖНО: предполагаем, что fetchAIResponse возвращает объект формата RecommendationsResponse
        // Например, для запроса "Рекомендации по темам" бэкенд вернёт мап из тематики -> { reply, book_search, vector_search }
        const res: unknown = await fetchAIResponse("Рекомендации по темам");

        // Защитно приводим тип к ожидаемому формату
        ...
        <div className="max-w-6xl mx-auto px-4 py-6 space-y-8">
          ...
          {!loading && !error && topics.length > 0 && (
            <div className="space-y-6">
              {topics.map((topic) => (
                <TopicSection key={topic} topic={topic} data={data[topic]} />
              ))}
            </div>
          )}
        </div>
